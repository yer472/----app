...
const RAW = [
  ...
  // ---------- 绘图（只在画板打开时） ----------
  {
    ...
    label: '画板工具：选择 / 直线 / 矩形 / 椭圆 / 手绘 / 模块 / 流向 / 橡皮',
    /*
     * ⚠️ 下面这句顺序说明**没有被自检断言**（自检只比 `keys` 和代码里的映射
     * 是否相等）。也就是说：改上面的 keys 却忘了改这句话，不会有任何提示。
     * 加工具时顺手核一遍。
     */
    caveat:
      '依次对应 V、L、R、O、P、N、F、E；按住 Shift 或 Alt 不算（Shift 是「约束角度」）',
  },
  {
    id: 'bd-undo',
    group: 'board',
